...
title: "Counso 开发者平台"
---
Counso 开发者平台帮助团队将 Agent 接入自己的应用和工作流程。你可以通过 API 以编程方式创建会话并使用工作区数据。[API 参考](/zh-Hans/docs/developer-platform/counso-api-documentation/openapi-and-postman)列出了可用请求。在相应开发环境中，也可以使用 JavaScript SDK。
...
根据配置，Agent 可以搜索资料、按时间倒序纳入最新文档，直到达到模型上下文上限，也可以查询表格或提取结构化数据。例如，可以通过 `POST https://app.counso.ai/api/v1/w/{wId}/assistant/conversations` 创建会话，再向 `/api/v1/w/{wId}/assistant/conversations/{cId}/messages` 发送消息。应用连接到其他 Counso 环境时，请替换基础地址。可用模型、集成和管理能力取决于工作区。开发工具说明见 [JavaScript SDK](/zh-Hans/docs/developer-platform/overview/javascript-sdk) 和 [API 参考](/zh-Hans/docs/developer-platform/counso-api-documentation/openapi-and-postman)。
