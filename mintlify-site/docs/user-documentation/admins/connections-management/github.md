...
Issue and pull-request labels are included with synchronized content. Additional labels identify the title, whether the item is a pull request, and the author, using forms such as `title:Fix search`, `isPullRequest:true`, and `author:@alex`. These can narrow [knowledge searches](/docs/user-documentation/agents/knowledge/search-data-sources).
