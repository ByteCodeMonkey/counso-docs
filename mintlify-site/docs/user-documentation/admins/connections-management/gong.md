...
4. After synchronization, make the data available in the relevant [Space](/docs/user-documentation/admins/spaces-management) so agents can use it.
...
Use these labels to filter the [knowledge search tool](/docs/user-documentation/agents/knowledge/search-data-sources), for example to focus on external calls in a particular language. Participant details are included only when Gong exposes them.
