...
The SDK follows a Result-based response pattern. Check `isErr()` before reading `value`, and handle API errors before continuing. The [Conversations API reference](/docs/developer-platform/counso-api-documentation/openapi-and-postman) lists request fields and available operations.
