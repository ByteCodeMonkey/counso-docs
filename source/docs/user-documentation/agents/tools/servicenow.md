...
| **`403 Forbidden`**                         | A REST API Access Policy, auth scope, table ACL, or field ACL rejected the call. Check each authorization layer separately (`rest_service`-type role, `itil`, table/field ACLs, access policies). Adding the `useraccount` scope to the OAuth application's **Auth Scopes** can also resolve API access errors. |
